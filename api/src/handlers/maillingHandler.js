...
async function welcomeEmail(email, displayName) {
  try {
    const info = await transporter.sendMail({
      ...
      to: email,
      subject: "Bienvenido a Rotela Shopping",
      html: `
        <h2> Bienvenido a Rotela Shopping ${displayName} </h2>
        <h3> Estamos muy felices que nos hayas escogido para tus futuras compras. </h3>
        <h4> Ir a Rotela Shopping haciendo clic aquí <a href="${homeURL}">ROTELA Shopping</a></h4>
      `,
    });
  ...
  try {
    const info = await transporter.sendMail({
      ...
      html: `
        ...
        <h3> Si estás recibiendo este email es porque tu compra en Rotela Shopping ha sido realizada con éxito.</h3>
